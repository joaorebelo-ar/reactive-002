    ...
    val suggestionSubscription: Subscription = suggestions.observeOn(eventScheduler) subscribe {
      x =>
        x match {
          ...
          case Failure(f)    => status.text = f.getLocalizedMessage
        }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map { click =>
      {
        val index = suggestionList.selection.leadIndex
        if (index > 0)
          suggestionList.listData(index)
        else
          ""
      }
